...
pub async fn read_tasks_with_submission_by_project_id(project_id: i32, user_id: String, conn: impl Executor<'_, Database=Postgres>) -> Result<Vec<RawTaskProposal>, sqlx::Error> {
...
mod test {
    ...
    async fn insert_task_test(pg_pool: PgPool) {
        ...
        let result2 = read_tasks_with_submission_by_project_id(1, String::from("user_id"), &pg_pool).await;
        println!("{:?}", result2);
